/**
 * Composable for managing domain intents
...
export function useDomainIntents() {
  ...
  async function invalidateCache() {
    ...
    try {
      await domainIntentsApi.invalidateCache(selectedDomain.value, organizationId.value)
      toast.add({
        severity: 'success',
        summary: 'Cache invalidado',
        detail: 'El cache de intents ha sido limpiado',
        life: 3000
      })
    } catch (error) {
