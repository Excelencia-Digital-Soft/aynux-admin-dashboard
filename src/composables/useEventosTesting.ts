...
export interface EventosTestMessage {
  id: string
  role: 'user' | 'assistant'
  content: string
  timestamp: string
...
export interface EventosWebhookConfig {
  enabled: boolean
  phoneNumber: string
  userName: string
  did: string | null
...
const DEFAULT_USER = 'Test Eventos'
const STORAGE_KEY = 'eventos-webhook-config'

export function useEventosTesting() {
  ...
  const messages = ref<EventosTestMessage[]>([])
  const inputMessage = ref('')
  const executionSteps = ref<ExecutionStep[]>([])
  const graphState = ref<unknown | null>(null)

  // Webhook Config
  const webhookConfig = ref<EventosWebhookConfig>({
    enabled: true,
  ...
  async function sendMessage(): Promise<void> {
    ...
    // Add user message
    const userMessage: EventosTestMessage = {
      id: `user-${Date.now()}`,
      role: 'user',
  ...
  function handleResponse(response: WebhookSimulationResponse): void {
    ...
    const assistantMessage: EventosTestMessage = {
  ...
  function updateWebhookConfig(updates: Partial<EventosWebhookConfig>): void {
  ...
  onMounted(() => {
    ...
    if (saved) {
      try {
        ...
        console.warn('Failed to parse eventos webhook config')
      }
    }
  })
